...
public interface ReservationDb extends JpaRepository<Reservation, String> {
    ...
    @Query("SELECT COUNT(r) FROM Reservation r WHERE r.room.id = :roomId AND " + "(:dateIn <= r.dateOut AND :dateOut >= r.dateIn)")
    int countReservationsInRange(@Param("roomId") String roomId, @Param("dateIn") Date dateIn, @Param("dateOut") Date dateOut);
